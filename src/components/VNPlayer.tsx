...
const VNPlayer: React.FC<VNPlayerProps> = ({ vnData, startSceneId, onComplete, onChoice, onMusicChange }) => {
  ...
  return (
    <div 
      className="fixed inset-0 bg-black overflow-hidden flex flex-col items-center justify-center cursor-pointer select-none"
      onClick={() => {
    ...
    >
        ...
        >
          {background && (
            <img 
              ...
              style={{ 
                transform: `${currentScene.flipBackgroundX ? 'scaleX(-1)' : ''} ${currentScene.flipBackgroundY ? 'scaleY(-1)' : ''}` 
              }}
              alt="Background" 
            />
          )}
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 w-full max-w-4xl z-[100]">
        <AnimatePresence>
          {isShowingChoices && (
            <motion.div
        ...
        >
          {/* Character Icon (if characterId is present) */}
          {currentDialogue?.characterId && (
            <div className="absolute -left-24 bottom-0 w-20 h-20 rounded-full border-4 border-white/20 overflow-hidden bg-black/40 backdrop-blur-md shadow-2xl">
              {(() => {
                const char = vnData.characters.find(c => c.id === currentDialogue.characterId);
                const expr = char?.expressions.find(e => e.name === currentDialogue.emotion) || char?.expressions[0];
                const asset = vnData.assets.find(a => a.id === expr?.assetId);
                return asset ? <img src={asset.url} className="w-full h-full object-cover" alt="Icon" /> : null;
              })()}
            </div>
          )}
          ...
          <div className="absolute bottom-4 right-6 flex items-center gap-4 text-zinc-500">
            <button onClick={(e) => { e.stopPropagation(); onComplete(); }} className="hover:text-white transition-colors p-2" title="Skip Scene">
              <SkipForward className="w-4 h-4" />
            </button>
            <button onClick={(e) => { e.stopPropagation(); setIsMuted(!isMuted); }} className="hover:text-white transition-colors p-2">
