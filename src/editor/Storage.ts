...
export const loadStagesFromDB = async (): Promise<SavedStage[]> => {
  ...
  return new Promise((resolve, reject) => {
    request.onsuccess = () => {
      const stages = request.result as SavedStage[];
      resolve(stages.sort((a, b) => (a.order ?? 0) - (b.order ?? 0)));
    };
    request.onerror = () => reject(request.error);
  });
};
...
export const loadWeeksFromDB = async (): Promise<SavedWeek[]> => {
  ...
  return new Promise((resolve, reject) => {
    request.onsuccess = () => {
      const weeks = request.result as SavedWeek[];
      resolve(weeks.sort((a, b) => (a.order ?? 0) - (b.order ?? 0)));
    };
    request.onerror = () => reject(request.error);
  });
};
