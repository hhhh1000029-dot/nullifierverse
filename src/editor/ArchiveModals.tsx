...
export const ArchiveImportModal: React.FC<{ 
  ...
  const handlePaste = async () => {
    try {
      ...
      // Silently fail and show manual input - the error is expected in iframes
      setShowPasteInput(true);
    }
  };

  const processPasteData = async (text: string) => {
    try {
      ...
      // Automatically select/import the pasted character into the lab
      onSelect(newChar);
    } catch (err) {
      console.error('Failed to process paste data:', err);
      alert('Failed to paste character. Make sure you have valid character data.');
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 bg-black/90 backdrop-blur-xl">
      ...
      >
        ...
        <AnimatePresence>
          {showPasteInput && (
            <motion.div 
              ...
              className="px-8 py-6 bg-zinc-950/50 border-b border-white/5"
            >
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-black text-pink-500 uppercase tracking-widest">Paste Character Data</h3>
                  <button onClick={() => setShowPasteInput(false)} className="text-zinc-500 hover:text-white transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <textarea 
                  ...
                  className="w-full h-32 bg-zinc-900 border border-white/10 rounded-2xl p-4 text-xs font-mono text-zinc-300 focus:outline-none focus:border-pink-500/50 transition-all custom-scrollbar"
                />
                <div className="flex justify-end gap-3">
                  <button 
                    onClick={() => setShowPasteInput(false)}
                    className="px-6 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-zinc-400 font-bold text-[10px] uppercase tracking-widest transition-all"
                  >
                    Cancel
                  </button>
                  <button 
                    onClick={() => processPasteData(manualPasteText)}
                    className="px-6 py-2 bg-pink-600 hover:bg-pink-500 rounded-xl text-white font-black text-[10px] uppercase tracking-widest transition-all"
                  >
                    Import Data
                  </button>
                </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Preview Section */}
          <div className="w-1/3 flex flex-col bg-zinc-900/50 p-8 overflow-y-auto custom-scrollbar">
            {selectedChar ? (
