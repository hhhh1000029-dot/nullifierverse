...
import { auth, loginWithGoogle, logout, onAuthStateChanged, User as FirebaseUser, isConfigValid } from './firebase';
...
export default function App() {
  ...
  const handleLevelComplete = (score: number, judgements: Judgements, maxCombo: number) => {
    ...
    if ((gameMode === 'CUSTOM_STORY' || gameMode === 'CUSTOM_STORY_PLAY') && storyProgress) {
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col items-center justify-center font-sans selection:bg-pink-500/30 overflow-hidden relative">
      <AnimatePresence mode="wait">
        ...
        {screen === 'STORY_CUTSCENE' && storyProgress && storyProgress.week.vnData && (
          <VNPlayer 
            key={`vn-player-${storyProgress.index}`}
            ...
            <div className="flex flex-col gap-3 flex-1">
              {[
                ...
                <motion.button
                  ...
                  onClick={() => {
                    if (item.id === 'ONLINE' && !isConfigValid) {
                      sfxManager.playHover(); // Play a different sound or just return
                      alert("Online Hub is disabled because Firebase is not configured. Please set up Firebase in the AI Studio settings to enable online features.");
                      return;
                    }
      ...
      {screen === 'PLAYING' && (
        ...
        >
          ...
          <RhythmGame 
            ...
            onRestart={() => {
              // Force restart by incrementing key and briefly changing screen
              setGameKey(prev => prev + 1);
              setScreen('START');
              setTimeout(() => {
                if (gameMode === 'EDITOR' && customStage) {
                  startGame(0, customStage, playbackRate, undefined, undefined, 'EDITOR');
      ...
      {screen === 'NEXT_SONG_RESULTS' && results && (
        <ResultsScreen
          ...
          onNext={() => {
            ...
            } else if ((gameMode === 'CUSTOM_STORY' || gameMode === 'CUSTOM_STORY_PLAY') && storyProgress) {
              handleNextStoryNode(storyProgress);
            }
          }}
          onBack={() => {
            if (gameMode === 'STORY') setScreen('STORY_MODE_MENU');
            else if (gameMode === 'CUSTOM_STORY' || gameMode === 'CUSTOM_STORY_PLAY') setScreen('STORY_MODE_MENU');
      ...
      {screen === 'LEVEL_COMPLETE' && results && (
        <ResultsScreen
          ...
          onNext={() => {
            if (gameMode === 'STORY') {
              setScreen('VICTORY');
            } else if ((gameMode === 'WEEK_PLAYTEST' || gameMode === 'CUSTOM_STORY' || gameMode === 'CUSTOM_STORY_PLAY') && playtestWeek) {
              setScreen('WEEK_RESULTS');
      ...
      {screen === 'WEEK_RESULTS' && playtestWeek && (
        <ResultsScreen
          ...
          onBack={() => {
            if (gameMode === 'CUSTOM_STORY' || gameMode === 'CUSTOM_STORY_PLAY') setScreen('STORY_MODE_MENU');
            else setScreen('EDITOR');
          }}
