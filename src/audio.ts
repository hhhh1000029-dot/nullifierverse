...
export class AudioEngine {
  ...
  async start(bpm: number, durationSec: number, audioUrl?: string, offset: number = 0) {
    ...
    const startTime = now + (leadIn / this.playbackRate) - (offset / this.playbackRate);
    ...
    if (this.isCustomAudio && this.audioBuffer) {
      ...
      this.sourceNode.start(now + (leadIn / this.playbackRate), offset);
    } else {
      ...
      for (let i = startBeat; i < totalBeats; i++) {
        ...
        const time = startTime + (songTime / this.playbackRate);
    ...
    // Update startTime again to account for any time spent scheduling
    // This ensures that when the game loop starts, it gets a fresh reference
