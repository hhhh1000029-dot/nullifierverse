...
import { loadStagesFromDB, saveStagesToDB, loadWeeksFromDB, saveWeeksToDB, loadArchiveCharacters, loadArchiveBackgrounds } from './Storage';
...
export const EditorMain: React.FC<EditorProps> = ({ onBack, onPlaytest, onPlaytestWeek, settings, setSettings, onEditStateChange }) => {
  ...
  const updateStage = (newStage: SavedStage, skipHistory = false) => {
    ...
    try {
      ...
      if (parsedData.chart) {
        ...
        if (targetId) {
          ...
          if (index !== -1) {
            const updatedStages = [...savedStages];
            // Keep the original ID and name, but update everything else
          ...
          newStage.name = newStage.name + ' (Copy)';
          const updatedStages = [...savedStages, newStage];
          setSavedStages(updatedStages);
          saveToLocalStorage(updatedStages);
    ...
    try {
      ...
      if (parsedData.week && parsedData.tracks) {
        ...
        if (targetId) {
          ...
          const updatedWeeks = [...savedWeeks, newWeek];
          setSavedStages(newStages);
          saveToLocalStorage(newStages);
          setSavedWeeks(updatedWeeks);
          saveWeeksToStorage(updatedWeeks);
  ...
  const exportWeek = (week: SavedWeek) => {
    showNotification('Compressing week data...');
    ...
    reader.onload = (e) => {
      try {
        ...
        if (parsedData.week && parsedData.tracks) {
          ...
          const newWeeks = [...savedWeeks, newWeek];
          ...
          const updatedStages = [...savedStages, parsedStage];
          setSavedStages(updatedStages);
          saveToLocalStorage(updatedStages);
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <button 
                            onClick={(e) => { e.stopPropagation(); setShowUploadModal(stage); }}
                ...
                savedWeeks.filter(w => w.name.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 ? (
                  ...
                  savedWeeks
                    ...
                    >
                      ...
                      <div className="relative shrink-0">
                        <div 
                          className={`w-14 h-14 rounded-xl overflow-hidden border-2 transition-all duration-500 ${
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
              ...
              >
                ...
                <div className="p-8 bg-zinc-950/50 border-t border-zinc-800 flex gap-4">
                  ...
                  <button 
                    onClick={() => uploadStage(showUploadModal)}
...
const WeekEditor: React.FC<{
  ...
  onBack: () => void;
  onPlaytestWeek: (week: SavedWeek, allStages: SavedStage[]) => void;
}> = ({ week, setWeek, savedStages, onSave, onBack, onPlaytestWeek }) => {
  const [isGeneratingThumbnail, setIsGeneratingThumbnail] = useState(false);
  const [thumbnailPrompt, setThumbnailPrompt] = useState('');
  ...
  if (activeMode === 'vn') {
    return <VNEditor week={week} setWeek={setWeek} onBack={() => setActiveMode('flow')} onSave={onSave} />;
  }
              ...
              return (
                <div key={node.id} className="bg-zinc-900 p-5 rounded-2xl border border-zinc-800 flex items-center justify-between group hover:border-zinc-700 transition-all">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${node.type === 'GAMEPLAY' ? 'bg-green-600/20 text-green-500' : 'bg-pink-600/20 text-pink-500'}`}>
                        {icon}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`text-[9px] font-black tracking-widest uppercase px-2 py-0.5 rounded-md ${node.type === 'GAMEPLAY' ? 'bg-green-600 text-white' : 'bg-pink-600 text-white'}`}>
                            {typeLabel}
                          </span>
                        </div>
...
const CharacterPreviewImage: React.FC<{ char: CharacterData; anim: Animation }> = ({ char, anim }) => {
  ...
  return (
    <img 
      ...
      style={{ 
        transform: `scale(${char.scale * (anim.scale || 1)}) ${char.flipX ? 'scaleX(-1)' : ''}`,
...
const CharacterLab: React.FC<{ stage: SavedStage; setStage: (s: SavedStage) => void }> = ({ stage, setStage }) => {
  ...
  const addExtra = (side: 'player' | 'opponent') => {
    ...
    setStage({ ...stage, extraCharacters: [...(stage.extraCharacters || []), newExtra] });
    setSelectedCharId(newExtra.id);
  };

  const removeExtra = (id: string) => {
    setStage({ ...stage, extraCharacters: (stage.extraCharacters || []).filter(ec => ec.id !== id) });
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col gap-1 mt-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-mono text-zinc-500">{char.scale.toFixed(2)}x</span>
              </div>
              <input 
                type="range" 
                min="0.1" 
                max="5" 
                step="0.1"
                value={Number.isNaN(char.scale) ? '' : char.scale}
...
const StageArchitect: React.FC<{ stage: SavedStage; setStage: (s: SavedStage) => void }> = ({ stage, setStage }) => {
  ...
  const updateStage = (updates: Partial<StageData>) => {
    setStage({ ...stage, stage: { ...stage.stage, ...updates } });
  };
  ...
  const handleImportBackground = (bg: ArchiveBackground) => {
    const newLayer: StageLayer = {
      ...
      scale: 1,
      position: { x: 0, y: 0 },
      zIndex: stage.stage.layers.length,
    };
    updateStage({ layers: [...stage.stage.layers, newLayer] });
  ...
  const updateLayer = (id: string, updates: Partial<StageLayer>) => {
    const newLayers = stage.stage.layers.map(l => l.id === id ? { ...l, ...updates } : l);
    updateStage({ layers: newLayers });
  };

  const removeLayer = (id: string) => {
    updateStage({ layers: stage.stage.layers.filter(l => l.id !== id) });
      ...
      <div className="w-80 bg-zinc-900 border-r border-zinc-800 p-6 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
        <div className="space-y-4">
            ...
            <input 
              ...
              onChange={(e) => {
                ...
                setStage({ 
                  ...stage, 
                  name: newName,
                  stage: { ...stage.stage, name: newName }
                });
              }}
              className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:border-cyan-500 transition-colors"
            />
          ...
          <div className="flex flex-col gap-1">
                    ...
                    if (file) {
                      ...
                      reader.onload = (event) => {
                        ...
                        img.onload = () => {
                          ...
                          setStage({ ...stage, thumbnail: base64 });
                        };
            ...
            {stage.thumbnail && (
              <button 
                onClick={() => setStage({ ...stage, thumbnail: undefined })}
        ...
        <div className="flex flex-col gap-3">
          {stage.stage.layers.sort((a, b) => b.zIndex - a.zIndex).map((layer) => (
            <div 
              key={layer.id} 
      ...
      <div className="flex-1 bg-zinc-950 relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative w-full h-full">
            {stage.stage.layers.sort((a, b) => a.zIndex - b.zIndex).map((layer) => (
              <div 
                key={layer.id}
...
const ChartMaster: React.FC<{ 
  ...
  const getDefaultEventValue = (type: string) => {
    switch (type) {
      ...
      case 'rotate': return { target: 'all', rotations: 1, duration: 4 };
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    const startStep = Math.max(0, Math.floor(target.scrollTop / 40) - 10);
    const endStep = Math.min(totalSteps, Math.ceil((target.scrollTop + target.clientHeight) / 40) + 10);
    ...
    if (isUserScrollingRef.current || !isPlayingPreview) {
      if (audioRef.current) {
        ...
        const currentStep = Math.max(0, (centerPixel - window.innerHeight * 0.5) / 40);
  ...
  const togglePreview = () => {
    ...
    if (isPlayingPreview) {
      ...
      const centerPixel = gridScrollRef.current ? gridScrollRef.current.scrollTop + gridScrollRef.current.clientHeight / 2 : 0;
      const currentStepAtPlay = Math.max(0, (centerPixel - window.innerHeight * 0.5) / 40);
      ...
      if (currentStepAtPlay < 0.1) {
        ...
        if (startPoint) {
          ...
          const targetScrollTop = startPoint.step * 40 + (window.innerHeight * 0.5) - (gridScrollRef.current?.clientHeight || 0) / 2;
      ...
      const updateScroll = () => {
        ...
        if (!isUserScrollingRef.current) {
          ...
          const targetScrollTop = currentStep * 40 + (window.innerHeight * 0.5) - gridScrollRef.current.clientHeight / 2;
  ...
  const updateChart = (updates: Partial<ChartData>) => {
    setStage({ ...stage, chart: { ...stage.chart, ...updates } });
  };
  ...
  const pasteSelectedArea = async () => {
    ...
    if (navigator.clipboard) {
      try {
        ...
        console.log('Using local copy state');
      }
    }
      ...
      <div className="bg-zinc-900 border-b border-zinc-800 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <input 
              type="number" 
              value={Number.isNaN(stage.chart.bpm) ? '' : stage.chart.bpm} 
          ...
          <div className="flex items-center gap-2">
            ...
            <input 
              type="number" 
              step="0.1"
              value={Number.isNaN(stage.chart.scrollSpeed) ? '' : stage.chart.scrollSpeed} 
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-zinc-950 rounded-lg border border-zinc-800">
            ...
            <input 
              ...
              onChange={(e) => setStage({ ...stage, audioUrl: e.target.value })} 
            ...
            <label className="bg-zinc-800 hover:bg-zinc-700 text-white px-2 py-1 rounded cursor-pointer flex items-center justify-center transition-colors shrink-0 ml-2">
              ...
              <input 
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    const reader = new FileReader();
                    reader.onload = () => setStage({ ...stage, audioUrl: reader.result as string });
                    reader.readAsDataURL(file);
                  }
                }} 
              />
                  ...
                  <>
                    ...
                    <div className="flex gap-4 items-center mt-1">
                      <div className="flex flex-col gap-2">
                        ...
                        <select 
                          value={event.value.movementType}
                          onChange={(e) => {
                            updateChart({
                    ...
                    <div className="flex gap-2">
                      <div className="flex flex-col gap-2 flex-1">
                        ...
                        <input 
                          ...
                          min="0"
                          max="1"
                          value={event.value.targetOpacity}
                          onChange={(e) => {
                            updateChart({
        ...
        {showEventMenu && (
          <>
            ...
            <motion.div 
              ...
              className="fixed z-50 w-96 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden"
              style={{ top: Math.min(showEventMenu.y, window.innerHeight - 300), left: 100 }}
            >
  ...
  return (
    <div className="h-full flex">
      <div className="w-80 bg-zinc-900/50 backdrop-blur-xl border-r border-white/5 p-6 flex flex-col gap-6 shadow-2xl shadow-black/50">
        ...
        <div className="grid grid-cols-2 gap-2">
          {['Camera Zoom', 'Flash Screen', 'Change Character', 'Play Animation', 'start_point', 'Rotate', 'Shader: Glitch', 'Shader: Chromatic', 'Shader: Lens', 'Shader: HUE', 'Shader: Gray', 'Add Text', 'Scroll Speed'].map(type => (
            <button 
              key={type}
              onClick={() => {
